...
function isRealmWorldStudioRendererProcess(commandLine, workingDirectory) {
  const normalized = normalizeForMatch(commandLine);
  const normalizedAppRoot = normalizeForMatch(appRoot);
  const normalizedWorkingDirectory = normalizeForMatch(workingDirectory);
  return (normalized.includes(normalizedAppRoot) || normalizedWorkingDirectory === normalizedAppRoot)
    && normalized.includes('vite')
    && normalized.includes('--port 1451');
}
...
function ensureRendererPortAvailable() {
  ...
  for (const pid of pids) {
    ...
    if (!isRealmWorldStudioRendererProcess(commandLine, workingDirectory)) {
      throw new Error(
        `Port ${rendererPort} is already in use by PID ${pid}. `
        + 'It is not a recognized Realm World Studio renderer process, so cleanup was skipped.',
      );
    }

    console.log(`[dev-renderer-port] Stopping stale Realm World Studio renderer on port ${rendererPort} (PID ${pid}).`);
    process.kill(pid);
  }
}
