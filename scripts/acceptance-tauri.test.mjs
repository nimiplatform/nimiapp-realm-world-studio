...
const rendererUrl = 'http://127.0.0.1:1451';
const cdpEndpoint = 'http://127.0.0.1:9223';
const evidenceDir = path.resolve(
  process.env.NIMI_ACCEPTANCE_OUTPUT_DIR
    || path.join(appRoot, '.nimi', 'local', 'acceptance', 'world-studio-tauri'),
);

test('real Tauri WebView2 shell matches the installed fail-close posture', { timeout: 180_000 }, async () => {
  ...
  try {
    ...
    await page.locator('[data-testid="world-studio-protected-session-failure"]').waitFor();
    await assertFailClosedPanel(page, 'capability-unavailable');
    ...
    assert.equal(bridgeProof.artifact.ok, false, JSON.stringify(bridgeProof.artifact));
    assert.ok(
      new Set([
      ...
      ]).has(bridgeProof.artifact.reasonCode),
      JSON.stringify(bridgeProof.artifact),
    );

    await page.getByTestId('world-studio-protected-session-retry').click();
    await assertFailClosedPanel(page, 'capability-unavailable');
    await page.getByRole('radio', { name: '中' }).click();
    await page.getByRole('heading', { name: 'World Studio 受保护操作尚未准入' }).waitFor();
    ...
    assert.doesNotMatch(narrowMetrics.bodyText, /�|锛|銆|鈥/u);
    await page.screenshot({
...
async function assertFailClosedPanel(page, expectedState) {
  const panel = page.getByTestId('world-studio-protected-session-failure');
  assert.equal(await panel.getAttribute('data-protected-state'), expectedState);
  assert.equal(await page.getByTestId('world-studio-protected-operations-locked').isDisabled(), true);
  assert.equal(await page.locator('.ras-shell').count(), 0);
  assert.equal(await page.getByRole('alert').count(), 1);
  assert.match(await panel.innerText(), /world-studio-protected-operation-set-not-admitted/u);
}
...
async function probeTauriBridge(page) {
  return page.evaluate(async () => {
    ...
    const probe = async (command, payload) => {
      try {
        const value = await hook.invoke(command, payload);
        ...
        try {
          ...
          // Native missing-command failures are intentionally plain strings.
        }
    ...
    return {
      ...
      artifact: await probe('nimi.shell.artifacts.readRuntimeBytes', {
        payload: { artifactId: 'world-studio-tauri-acceptance-probe' },
      }),
    };
  });
}
...
async function assertCdpPortAvailable() {
  try {
    ...
    if (response.ok) throw new Error('WebView2 CDP port 9223 is already in use');
  } catch (error) {
