...
const evidenceDir = path.resolve(
  process.env.NIMI_ACCEPTANCE_OUTPUT_DIR
    || path.join(appRoot, '.nimi', 'local', 'acceptance', 'world-studio-electron'),
);

test('real Electron shell keeps unadmitted World Studio operations fail-closed', { timeout: 90_000 }, async () => {
  ...
  return page.evaluate(async () => {
    ...
    return {
      ...
      artifact: await probe('nimi.shell.artifacts.readRuntimeBytes', {
        payload: { artifactId: 'world-studio-electron-acceptance-probe' },
      }),
    };
  });
}
...
async function setContentSizeAndInspect(electronApp, page, width, height) {
  ...
  for (let attempt = 0; attempt < 4; attempt += 1) {
    await electronApp.evaluate(({ BrowserWindow }, size) => {
      ...
      if (!window) throw new Error('Realm World Studio BrowserWindow unavailable');
