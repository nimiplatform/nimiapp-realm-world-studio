...
const rendererUrl = process.env.NIMI_REALM_WORLD_STUDIO_ELECTRON_RENDERER_URL || 'http://127.0.0.1:1451';
...
async function waitForUrl(url, timeoutMs) {
  ...
  throw new Error(`Timed out waiting for Realm World Studio renderer at ${url}: ${lastError instanceof Error ? lastError.message : String(lastError || '')}`);
}
