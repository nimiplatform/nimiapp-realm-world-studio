...
- **[R-RWS-RUNTIME-010]** The current installed-app bootstrap is transitional and artifact-only. It may consume the shared protected artifact carrier, but it must not construct a generic Runtime client, account caller, Realm transport, release descriptor, launch nonce, endpoint override, or renderer-owned session authority while the World Studio operation set is unadmitted.
- **[R-RWS-RUNTIME-011]** Electron and Tauri hosts use the shared native installed carrier. Host identity and release trust cannot come from renderer metadata, environment variables, command-line arguments, app-local files, or ordinary localhost gRPC; a platform without an admitted native carrier remains explicitly fail-closed.
